...
**姊妹篇（推荐一并跑验收）：** [ASMR_LITE_BENCHMARK_V2.1.md](./ASMR_LITE_BENCHMARK_V2.1.md) — 歧视性样本（**OneLink-L1 > 本地 lexical**）、**`entity_hits` 断言**与 **`query_preference_polarity` 开放式问句校验**，脚本：`scripts/benchmark-asmr-lite-v2.1.sh`。v2/v2.1 runner 共享 `scripts/lib/asmr-benchmark-v2-common.sh`。
...
- 与 `OneLink/docs/archive/rules-legacy-2026-05-15/Rules-V2/EXECUTION/asmr-lite-benchmark-and-acceptance-checklist.md` 中远期「向量 / 单次大模型」对照组 **不是同一含义**；v2 仅指 **本仓库 shell scaffold**。
...
- `retrieval_modes`（与本次 `context/build` 经策略过滤后的实际检索模式一致）

并断言响应体含 **`policy_version_label`**；若已有聊天写入的 summary（`summary_count > 0`），则 **`latest_summary_policy_version`** 非空。
