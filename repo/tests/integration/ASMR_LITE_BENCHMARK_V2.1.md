...
- **`query_preference_polarity`** 对开放式偏好问句回落 **`neutral`**、对「是否不喜欢…」保留 **`negative`** 等，可与 observability **严格断言**（`query_polarity_open.json`）。
...
| `query_polarity_open.json` | **Query-side 偏好极性**：开放式偏好问句要求 `query_preference_polarity == neutral`；「是否不喜欢…」类要求 **`negative`**；同时 `expected_contains` 仍须命中记忆中的偏好表述。 |
...
- `expect_query_preference_polarity`：`positive` \| `negative` \| `neutral`，与 `routing.last_observation.query_preference_polarity` 严格相等（若省略则不校验）。
...
**重要**：二者与 `OneLink/docs/archive/rules-legacy-2026-05-15/Rules-V2/EXECUTION/asmr-lite-benchmark-and-acceptance-checklist.md` 中远期「向量 / 单次 LLM」类 Baseline-A/B **不是同一东西**；v2/v2.1 文档与脚本注释中的 baseline **仅指本仓库 shell scaffold**。
...
- 若配置了 `expect_query_preference_polarity`，打印 **`query_preference_polarity` 校验 OK**

v2 与 v2.1 runner 共享 **`scripts/lib/asmr-benchmark-v2-common.sh`**（lexical baseline、`build_context`、observability 形状断言等），入口脚本路径不变。
...
`summary_hits`、`artifact_hits`、`entity_hits`、`route_confidence`、`estimated_llm_calls`、`estimated_tokens`、`query_preview`、`upgraded`、`query_preference_polarity`、`evidence_preference_polarity`、**`retrieval_modes`**（须为 **array**；与当次 `build_context` 的 **`retrieval_used`** 一致）。

共享的 **`assert_observation_shape`**（`scripts/lib/asmr-benchmark-v2-common.sh`）另校验根级 **`policy_version_label`**（非空字符串）；若 **`summary_count > 0`**（套件在 setup 后通常满足），则 **`latest_summary_policy_version`** 须非空。

其中 **`evidence_preference_polarity`** 反映当前最高分证据上的蒸馏极性；**`query_preference_polarity`** 为 **query 侧**启发式（开放式偏好问句常为 `neutral`，嵌入「是否不喜欢」等可为 `negative`），**不要**与 artifact 上的 `preference_polarity` 混为一谈。详见 `OneLink/docs/archive/rules-legacy-2026-05-15/Rules-V2/CONTRACTS/context-service-contract.md` 与 `context-service` README。
...
4. 抽查 `memory_context` 中含 `activation_top` / `query_polarity_hint` / `pref_top` 等实现已写入的片段（与 contract 一致即可）。
5. 跑 `query_polarity_open` 套件时，确认 **`query_preference_polarity` 断言**与实现一致（开放式 → `neutral`，「是否不喜欢…」→ `negative`）。
