...
const SERVICE_NAME: &str = "ai-chat-service";
...
pub async fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if config.env_mode != "dev" && config.database_url.is_none() {
        return Err(format!(
            "ai-chat-service: FATAL: DATABASE_URL is not set in non-dev environment (ONELINK_ENV={}). \
    ...
    let persistence = store::PersistenceBackend::from_config(config.database_url.as_deref()).await;
    let pg_store = match &persistence {
        store::PersistenceBackend::Postgres(pg) => {
            tracing::info!(backend = %persistence, "ai-chat-service: persistence backend active");
            Some(Arc::new(pg.clone()))
        }
    ...
    let app = axum::Router::new()
        .merge(health::router())
        .merge(http::routes::router(config.clone(), pg_store));
