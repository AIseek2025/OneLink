...
pub fn router(config: Config, pg: Option<Arc<PostgresStore>>) -> Router {
    let state = Arc::new(AppState::new(config, pg));

    let public_and_internal = Router::new()
        .route(
    ...
    public_and_internal.merge(observability)
}
...
async fn create_or_get_conversation(
    ...
    if let Some(ref pg) = state.pg_store {
        if let Some(cid) = pg
            .get_user_primary_conversation(&user_id)
            .await
...
async fn send_message(
    ...
    if let Some(ref pg) = state.pg_store {
        let owner = pg
            ...
            .await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("db: {e}")))?;
        let owner = owner.ok_or((StatusCode::NOT_FOUND, "conversation not found".to_string()))?;
        if owner != user_id {
            return Err((StatusCode::FORBIDDEN, "not your conversation".to_string()));
        }

        pg.insert_conversation(&ConversationRow {
            conversation_id: conversation_id.clone(),
            user_id: user_id.clone(),
            status: "active".to_string(),
            created_at: DEFAULT_TIMESTAMP.to_string(),
    ...
    if let Some(ref pg) = state.pg_store {
        pg.insert_message_with_content(
            &conversation_id,
            &MessageRow {
        ...
        let current_cv: i32 = {
            ...
            conversations
                .get(&conversation_id)
                .map(|c| c.context_version as i32 + 1)
                .unwrap_or(1)
        };
        pg.update_conversation_last_message(&conversation_id, current_cv)
            .await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("db: {e}")))?;
    }
    ...
    // contexts HashMap is ephemeral session state (context_version, summary) not persisted to PG;
    // always update it regardless of PG mode so get_context returns current view.
    {
    ...
    let checkpoint_user_id = if let Some(ref pg) = state.pg_store {
        pg.get_conversation_owner(&conversation_id)
            .await
            .ok()
            .flatten()
            .unwrap_or_default()
...
async fn get_internal_message(
    ...
    if let Some(ref pg) = state.pg_store {
        ...
        let owner_user_id = pg
            .get_conversation_owner(&conversation_id)
            .await
            .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
            .ok_or(StatusCode::NOT_FOUND)?;
        return Ok(Json(InternalMessageResponse {
            conversation_id,
            owner_user_id,
...
struct AppState {
    ...
    // Ephemeral session state: context_version and summary are not persisted to PG.
    // This HashMap is the authoritative source for context view even when PG is active.
    contexts: Mutex<HashMap<String, ContextView>>,
    relay_failures: Mutex<Vec<RelayFailureRecord>>,
}

impl AppState {
    fn new(config: Config, pg_store: Option<Arc<PostgresStore>>) -> Self {
        Self {
            config,
