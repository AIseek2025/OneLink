//! OneLink `safety-service` — content safety, reports, blocks, and DM review.
...
const SERVICE_NAME: &str = "safety-service";
...
pub async fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if config.env_mode != "dev" && config.database_url.is_none() {
        return Err(format!(
            "safety-service: FATAL: DATABASE_URL is not set in non-dev environment (ONELINK_ENV={}). \
    ...
    let state = http::routes::SafetyState::new(config.clone(), pg);
