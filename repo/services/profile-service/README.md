...
- **完成度** `GET /api/v1/profile/me/completion` 仍为 **五维**结构化覆盖率（维度集合与 Phase A 一致）。此为**画像**口径，与 **question-service** 的问卷进度（`/questions/status`、`/questions/completion`）**不联动**；勿将 `profile/me/completion` 称为问卷完成度。**BFF `GET /api/v1/bff/onboarding`** 的 **`progress`** 字段亦 **仅**问卷域透传，**不**映射自本接口。
...
## 联调纵切面
见 `repo/tests/integration/CHAT_MEMORY_PROFILE_SLICE.md`。当前开发入口见 `OneLink/rules/09-NEXT-DEVELOPMENT-PLAN.md`；旧多代理发单索引已归档到 `OneLink/docs/archive/rules-legacy-2026-05-15/Rules-V2/EXECUTION/README.md`。后续 ASMR-Lite benchmark 历史口径见 `OneLink/docs/archive/rules-legacy-2026-05-15/Rules-V2/EXECUTION/asmr-lite-benchmark-and-acceptance-checklist.md`（本服务仅预留日志/事件名观察，不跑完整 benchmark）。
...
历史依据：`OneLink/docs/archive/rules-legacy-2026-05-15/Rules/10-SERVICE-BOUNDARIES.md`, `OneLink/docs/archive/rules-legacy-2026-05-15/Rules/11-DATA-EVENT-MODEL.md`, `OneLink/docs/archive/rules-legacy-2026-05-15/Rules/15-MVP-OPENAPI-DRAFT.md`, `OneLink/docs/archive/rules-legacy-2026-05-15/Rules/16-MVP-EVENT-SCHEMAS-DRAFT.md`
