...
fn opt_msg(input: &MemoryResolveInput) -> Option<&str> {
    input
        .source_message_id
        .as_deref()
        .map(|s| s.trim())
...
fn content_suggests_goal(c: &str) -> bool {
    c.contains("希望认识")
        || c.contains("想认识")
        || c.contains("要认识")
        || (c.contains("投资人") && (c.contains("希望") || c.contains("认识") || c.contains("想")))
...
fn content_suggests_location(c: &str, network_type: &str, keywords: &[String]) -> bool {
    ...
    if keywords.iter().any(|k| KNOWN_CITIES.contains(&k.as_str())) {
        return true;
    }
...
fn explicit_communication_preference_value(c: &str) -> Option<String> {
    ...
    if c.contains("拐弯抹角")
        && (c.contains("不喜欢") || c.contains("讨厌") || c.contains("别") || c.contains("不要"))
    {
        return Some(c.to_string());
    }
...
fn infer_facts_one(input: &MemoryResolveInput) -> Vec<StoredFact> {
    ...
    if let Some(v) = explicit_communication_preference_value(c) {
        push_fact(&mut out, FACT_COMMUNICATION_PREFERENCE, v, mid, msg, 0.86);
    }
...
pub fn traits_from_facts(facts: &[StoredFact]) -> TraitSnapshot {
    ...
    for f in facts {
        match f.fact_type.as_str() {
            FACT_INTEREST if !interest_tags.contains(&f.value) => {
                interest_tags.push(f.value.clone());
            }
            FACT_GOAL if !connection_goal_tags.contains(&f.value) => {
                connection_goal_tags.push(f.value.clone());
            }
            FACT_LOCATION if location_label.is_none() => {
                location_label = Some(f.value.clone());
            }
            FACT_COMMUNICATION_PREFERENCE if !communication_preferences.contains(&f.value) => {
...
pub fn refresh_derived_profile_fields(
    ...
    *bio = if bio_parts.is_empty() {
    ...
    for f in facts.iter().take(20) {
        memory_highlights.push(format!("[{}] {}", short_type_label(&f.fact_type), f.value));
    }
}
...
mod tests {
    ...
    fn location_pattern_extracts_non_hardcoded_city() {
        ...
        let loc = facts
            .iter()
            .find(|f| f.fact_type == FACT_LOCATION)
            .expect("location");
    ...
    fn smoke_like_messages_produce_structured_facts_and_traits() {
        ...
        let interest_n = facts
            .iter()
            .filter(|f| f.fact_type == FACT_INTEREST)
            .count();
        assert!(
            interest_n >= 2,
        ...
        assert_eq!(
            comm.len(),
            1,
            "expected one explicit communication fact from m4"
        );
