...
fn spawn_profile_service(port: u16) -> ServiceGuard {
    let bin = std::env::var("PROFILE_SERVICE_BIN").unwrap_or_else(|_| {
        let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap();
        format!("{manifest_dir}/../../target/debug/profile-service")
    });
...
#[test]
fn profile_service_process_level_auth_rejection() {
    if std::env::var("RUN_PROCESS_LEVEL_SMOKE").ok().as_deref() != Some("1") {
        eprintln!("skip: set RUN_PROCESS_LEVEL_SMOKE=1 to run process-level tests (requires profile-service binary)");
        return;
    }

    let port: u16 = 19094;
    let guard = spawn_profile_service(port);

    assert!(
        wait_for_health(port, Duration::from_secs(15)),
        "profile-service did not become healthy on port {port}"
    );
...
#[test]
fn profile_service_rejects_default_secret_in_staging() {
    if std::env::var("RUN_PROCESS_LEVEL_SMOKE").ok().as_deref() != Some("1") {
    ...
    assert!(
        !status.success(),
        "profile-service must NOT start with default secret in staging env, but it exited successfully"
    );
}

#[test]
fn profile_service_rejects_short_secret_in_production() {
    if std::env::var("RUN_PROCESS_LEVEL_SMOKE").ok().as_deref() != Some("1") {
        eprintln!("skip: set RUN_PROCESS_LEVEL_SMOKE=1 to run process-level negative tests");
        return;
    }
    ...
    assert!(
        !status.success(),
        "profile-service must NOT start with short secret in production env, but it exited successfully"
    );
}
