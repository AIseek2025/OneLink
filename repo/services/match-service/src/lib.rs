//! OneLink `match-service` — candidate recommendation engine.
...
const SERVICE_NAME: &str = "match-service";
...
pub async fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if config.env_mode != "dev" && config.database_url.is_none() {
        return Err(format!(
            "match-service: FATAL: DATABASE_URL is not set in non-dev environment (ONELINK_ENV={}). \
    ...
    let state = http::routes::MatchState::new(config.clone(), pg);
