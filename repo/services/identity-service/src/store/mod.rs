        ...
        match database_url {
            Some(url) if !url.trim().is_empty() => {
                tracing::info!(
                    ...
                    "identity-service: Postgres persistence configured"
                );
                match PostgresStore::connect(url).await {
                    Ok(store) => PersistenceBackend::Postgres(store),
                    Err(e) => {
                        tracing::error!(error = %e, "identity-service: Postgres connect failed — refusing silent in-memory fallback in shared environment");
                        panic!(
                            "identity-service: FATAL: Postgres connect failed (DATABASE_URL was set). \
            ...
            _ => {
                tracing::info!(
                    "identity-service: in-memory mode (DATABASE_URL unset) — dev/smoke only"
                );
                PersistenceBackend::InMemory(memory::InMemoryStore::new())
            }
        }
    }
...
impl fmt::Display for PersistenceBackend {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PersistenceBackend::InMemory(_) => write!(f, "in-memory"),
