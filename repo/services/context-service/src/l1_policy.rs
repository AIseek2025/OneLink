...
/// 查询是否在问「偏好/沟通」类信息（与 [`INTENT_WANTS_PREFERENCE`] 同口径，供外部排查影响面）。
#[inline]
pub fn query_mentions_preference_intent(query: &str) -> bool {
    INTENT_WANTS_PREFERENCE.iter().any(|m| query.contains(*m))
}

/// 开放式偏好问句：在问「什么样 / 哪种 / 如何」等，query 文本本身不应被当成用户的 positive/negative 立场。
#[inline]
fn is_preference_open_question(query: &str) -> bool {
    if query.contains("什么样") || query.contains("哪种") {
        return true;
    }
    if query.contains("如何") || query.contains("怎样") {
        return true;
    }
    // WH-问法：喜欢什么 / 不喜欢什么 / 想要什么 …
    if query.contains("喜欢什么")
        || query.contains("不喜欢什么")
        || query.contains("想要什么")
        || query.contains("讨厌什么")
    {
        return true;
    }
    if query.contains("偏好什么") || query.contains("偏好哪") {
        return true;
    }
    // 「A 还是 B」类选择问句：不是 query 自己在表达单一极性
    ...
    // 「沟通方式」+ 疑问色彩
    if query.contains("沟通方式")
        && (query.contains("什么") || query.contains("哪") || query.contains("怎样"))
    {
        return true;
    }
    false
...
///
/// 与 [`infer_preference_polarity`] 解耦：开放式问句回落 `neutral`；嵌入「是否不喜欢/是否喜欢…」等立场时再走极性推断。
#[inline]
pub fn query_preference_polarity(query: &str) -> Option<&'static str> {
    if !query_mentions_preference_intent(query) {
        return None;
    }
    if is_preference_open_question(query) {
        return Some("neutral");
    }
...
pub const SCORE_GRAPH_EXPANSION_BASE: f64 = 0.28;
pub const SCORE_SUPERSEDES_GENERAL: f64 = 0.04;
pub const SCORE_CONFIDENCE_DIVISOR: f64 = 10.0;
pub const ACTIVATION_DECAY_RATE_DEFAULT: f64 = 0.3;
pub const SCORE_ACTIVATION_WEIGHT_DEFAULT: f64 = 0.12;
pub const IMPORTANCE_SCORE_DEFAULT: f64 = 0.5;
...
#[cfg(test)]
mod query_polarity_tests {
    use super::*;

    #[test]
    fn open_question_likes_style_is_neutral_not_positive() {
        assert_eq!(
            query_preference_polarity("用户喜欢什么样的沟通方式？"),
            Some("neutral")
        );
    }

    #[test]
    fn embedded_dislike_in_question_stays_negative() {
        assert_eq!(
            query_preference_polarity("用户是否不喜欢被推销式沟通？"),
            Some("negative")
        );
    }

    #[test]
    fn choice_question_is_neutral() {
        assert_eq!(
            query_preference_polarity("用户喜欢直接的还是委婉的沟通？"),
            Some("neutral")
        );
    }

    #[test]
    fn non_preference_query_returns_none() {
        assert_eq!(query_preference_polarity("用户现在在哪个城市？"), None);
    }
}
