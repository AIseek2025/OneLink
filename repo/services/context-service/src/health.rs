...
pub fn router() -> Router {
    Router::new()
        .route(
            "/health",
            get(|| async { Json(json!({"status": "alive", "service": "context-service"})) }),
        )
        .route(
            "/ready",
            get(|| async { Json(json!({"status": "ready", "service": "context-service"})) }),
        )
        .route(
            "/metrics",
            get(|| async { Json(json!({"service": "context-service"})) }),
        )
}
...
mod tests {
    ...
    async fn test_metrics_returns_service() {
        ...
        let bytes = resp.into_body().collect().await.unwrap().to_bytes();
        let json: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(json["service"], "context-service");
