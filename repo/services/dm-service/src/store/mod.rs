    ...
    pub async fn from_config(database_url: Option<&str>) -> Self {
        match database_url {
            Some(url) if !url.trim().is_empty() => match PostgresStore::connect(url).await {
                Ok(pg) => {
                    tracing::info!("dm-service: connected to Postgres");
                    PersistenceBackend::Postgres(pg)
                }
                Err(e) => {
                    tracing::error!(error = %e, "dm-service: Postgres connect failed — refusing silent in-memory fallback");
                    panic!(
                        "dm-service: FATAL: Postgres connect failed (DATABASE_URL was set). \
            ...
            _ => {
                tracing::info!(
                    "dm-service: no DATABASE_URL, using in-memory store — dev/smoke only"
                );
